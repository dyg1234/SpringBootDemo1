;(function ($, window, document, undefined) {
    ...
    var remote={
         ...
         */
        getImgName : function(callback) {
            $.ajax({
                url: "/getcode", //请求的url地址
    ...
    Slide.prototype = {
        ...
        init: function() {
            ...
            // 获取滑动验证码背景图
            remote.getImgName( function (ret) {
...
//				tmpHtml = '<canvas class="verify-sub-block"  width="'+this.plusWidth+'" height="'+this.plusHeight+'" style="left:0; position:absolute;" ></canvas>';
                tmpHtml = '<img class="verify-sub-block" src="' + this.options.smallImg + '" width="'+this.plusWidth+'" height="'+this.plusHeight+'" style="position:absolute;"/>';
            }
            ...
            if(this.options.mode == 'pop') {
                ...
                this.htmlDoms.out_panel.css({ 'position': 'absolute','z-index':'4', 'bottom': this.options.barSize.height});
                this.htmlDoms.sub_block.css({'display': 'none','z-index':'5',});
            }else {
...
//          this.y = top;
            ...
            if(this.options.mode == 'pop') {
                ...
                this.htmlDoms.sub_block.css({'top': '-'+this.setSize.img_height +'px'});
            }else {
        ...
        refresh: function(flag) {
            var _this = this;
            // 初始化时调用
            !flag && remote.getImgName(function(ret) {
